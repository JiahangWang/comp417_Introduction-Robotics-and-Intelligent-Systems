...
class InvertedPendulumGame(object):
    ...
    def game_round(self):
        ...
        if self.RL_controller is not None and self.game_round_number % 10 == 0:
            state_value_matrix = self.RL_controller.get_state_value_matrix()
            with open("matrix.txt", "a") as file:
                file.write("State Value Matrix after game round {}:\n".format(self.game_round_number))
                np.savetxt(file, state_value_matrix, fmt='%-7.2f')
                file.write("\n")
